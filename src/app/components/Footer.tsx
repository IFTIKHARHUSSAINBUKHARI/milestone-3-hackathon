import React from 'react';

export default function Footer() {
  return (
    <div className="relative flex flex-col items-center justify-center gap-6 px-5 w-full bg-white py-6">
    {/* Intro Section */}
    <div className="w-full flex flex-wrap items-start justify-between gap-6">
      <div className="flex flex-col gap-2 w-full lg:w-auto">
        <h1 className="text-[#3563e9] text-2xl lg:text-3xl font-bold text-center lg:text-left">MORENT</h1>
        <p className="text-gray-500 text-center lg:text-left w-[300px]">
          Our vision is to provide convenience and help increase your sales business.
        </p>
      </div>
  
      {/* Lists Section */}
      <div className="flex flex-wrap gap-6 justify-center lg:justify-between w-full lg:w-auto lg:mr-10">
        {[
          { title: "About", items: ["How it works", "Featured", "Partnership", "Business Relation"] },
          { title: "Community", items: ["Events", "Blog", "Podcast", "Invite a friend"] },
          { title: "Socials", items: ["Discord", "Instagram", "Facebook", "Twitter"] },
        ].map((list, idx) => (
          <div key={idx}>
            <ul className="flex flex-col gap-1">
              <li className="font-bold text-lg">{list.title}</li>
              {list.items.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
        ))}
      </div>
    </div>
  
    {/* Divider Line */}
    <div className="border-t w-full border-[#e7eef6]"></div>
  
    {/* Footer Section */}
    <div className="w-full flex flex-wrap items-center justify-between gap-4">
      <h1 className="font-bold text-sm text-center lg:text-left w-full lg:w-auto">
        ©2022 MORENT. All rights reserved
      </h1>
      <div className="flex flex-wrap justify-center lg:justify-end items-center gap-4 w-full lg:w-auto">
        {["Privacy & Policy", "Terms & Conditions"].map((text, idx) => (
          <h1 key={idx} className="font-bold text-sm">
            {text}
          </h1>
        ))}
      </div>
    </div>
  </div>
  
  );
}
